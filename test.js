...
// MQTT spec 1.5.5 Variable Byte Integer
// max size 24 bits
...
// command specific tests start here
...
testParseGenerate('minimal connect with clientId as UTF8', {
  cmd: 'connect',
  retain: false,
  qos: 0,
  dup: false,
  ...
  0, // Connect flags
  0, 30, // Keepalive
  0, 9, // Client ID length
  116, 101, 115, 116, 32, 240, 159, 153, 136 // Client ID
]))

testParseGenerate('minimal connect on 3.1.1 with clean set and no clientId', {
  cmd: 'connect',
  retain: false,
  qos: 0,
  dup: false,
...
testParseError('Packet too short', Buffer.from([
  32, 0 // Connack Header only, no payload
]))
...
testGenerateOnly('publish MQTT 5 properties with 0-4 byte varbyte', {
  cmd: 'publish',
  retain: true,
  qos: 2,
  dup: true,
...
testParseGenerate('pubrel MQTT 5 properties', {
  cmd: 'pubrel',
  retain: false,
  qos: 1,
  dup: false,
...
testWriteToStreamError('Invalid topic', {
  cmd: 'publish',
  topic: ''
}, {}, 'Invalid topic: empty string')

testWriteToStreamError('Invalid topic', {
  cmd: 'publish',
  topic: Buffer.alloc(0)
}, {}, 'Invalid topic: empty buffer')
